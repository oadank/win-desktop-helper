...
## ZCode/Electron 冻结恢复路径 + tray_click 用法

冻结症状: 窗口可见、Win32 样式正常、UIA 树能读, 但 UIA invoke / 真实鼠标点击全无反应(Electron 内部输入路由死, activate 唤回救不活)。已验证恢复路径: win_manage action=close 关窗(进程退托盘不死) -> tray_click name=应用名 double=1 双击重开 -> 完全恢复(实测 ZCode 双向切页流畅)。tray_click 行为: 单击=toggle(可见时最小化), 双击=恢复/打开; via=taskbar 时图标在任务栏不在托盘区。verify 注意: 旧版只对比按钮自身像素, 按钮外观不变(切页签)会误报 changed:false — 2026-09-07 10:13 后已改整窗对比(窗口中间80%)。ui_find 拿到的 rect 若明显不在窗口范围(x=9 而窗口在 x=1280) = UIA 返回窗口相对坐标, ui_click mode=coord 已自动平移修正, 且 HitGuard 会拦截点飞。
