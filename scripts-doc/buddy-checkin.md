...
5. 「立即领取」定位：面板截图找**最下一条文字行**，取该行**首个文字块**中心（4 字按钮，已领/未领两态几何一致、通用）
   🔴 **2026-09-22 作废**：该行**首块不一定是按钮**（9 期面板该行是「今日可领100积分」文字 + 「立即领取」按钮两块）→ 见文末「按钮必须认字」
...
## 🔴 2026-09-20 又两个真根因：侧栏收起态 + 菜单第三字被切（已修，实测 13.9s）

**当天结果**：面板 已领4天/400分 → **已领5天/500分**（+100 到账），按钮「立即领取」→「今日已领」。

> ⚠️ 上一版本节说「绿圆实测 **100% 命中**」—— **该结论过于绝对，已修正**：
> 那是在**侧栏展开态**下测的。侧栏收起时找不到头像，**100% 不命中**。
> 教训：把"100% 命中"写进手册前，先问「什么条件下会不命中」。

### 🔴 新根因 1：侧栏收起时，账号头像**根本不渲染**（不是坐标问题）
侧栏收起后，窗口左下角是**聊天输入框**（`+` 按钮 / `允许完全访问`），**头像不在屏幕上**。
此时 `find_avatar` 正确返回 `None`，而旧代码的「兜底估位 `(rx+40, ry+rh-50)`」盲点一下 →
落到输入框 → 菜单不开 → OCR 读到的是**消息区**文字（`WB-TICKETS-0919.m / 查看所有产物(4) /
复制 点赞 点踩 / 今天帮你做些什么 / 允许完全访问`）→ `stage=open_menu` 失败。

**诊断要点**：只要 `menu_lines` 里出现「查看所有产物」「今天帮你做些什么」这类**聊天区文字**，
就说明**头像没点开**，别去调菜单探区参数 —— 先查侧栏是不是收起了。

**修法**：新增 `ensure_sidebar()` —— 先找左下角绿圆；扫不到就点**左上角侧栏开关**
（现场扫该区域最靠左的前景块，不写死坐标；实测命中 `(44,34)`）展开，再复查绿圆。
另：`loc_avatar` 扫不到**直接返回 None 明确报错**（`stage=avatar`），
**不再盲猜坐标** —— 盲猜必落输入框，还会把日志带偏。

### 🔴 新根因 2：`MENU_TEXT_X` 右边界 250 把「Buddy加油站」的「站」切掉了
侧栏展开后菜单能打开了，但 OCR 仍是 0 命中，读出 `'Buddy加油'`。
实测该行文字**跨 x=64~286**，而右边界 250 把「站」截断 → 关键词 `"加油站"` 匹配不上。
（右侧浅灰说明列从 x≈400 才开始，所以放宽到 320 是安全的。）

**修法**：`MENU_TEXT_X` 右边界 `250 → 320`；并加关键词兜底
`any(k in t1 for k in ("加油站", "Buddy加油", "Buddy 加油", "加油"))`。

### 🔴 附带修正：`MENU_PROBE_H` 700 → 820
700 会把探区**最上面那行**（恰好就是「Buddy加油站」）切在框外 → 只读到
「去邀约/成长计划/设置/记忆与进化/外观/帮助与反馈」。实测菜单顶部到 Buddy加油站行约 720~760px。

### 本次实测真值（2560x1440@125%、侧栏**展开**、贴左后）
| 元素 | 屏幕坐标 |
|---|---|
| 侧栏开关（左上角，竖条图标） | (40, 29) |
| 左下角头像（绿圆） | **(80, 1256)** |
| 菜单「Buddy加油站」 | (174, 540)（现场算出） |
| 面板「立即领取」 | ~(166, 1124) |

注：窗口 rect 报 **1800x1332**（`/win/snap?pos=left` 返回 ok 但**未真正半屏**），
**不要依赖 rect 推坐标** —— 这也是"必须现场定位"的又一佐证。

### 泛训（可迁移）
- **元素找不到时，先问「它现在渲染了吗」**，再怀疑定位算法。UI 状态（侧栏收起/面板未开）
  会让元素**不存在**，而"坐标算错"是两回事 —— 前者无论怎么调参数都点不中。
- **OCR 关键词匹配要考虑截断**：中文词被裁掉尾字（`Buddy加油站`→`Buddy加油`）会让子串匹配失效。
  关键词用**更短的稳定前缀**兜底，比延长裁剪边界更鲁棒（两者一起做更好）。


## 🔴 2026-09-22 第三个真根因：领取按钮不能靠「块序」猜，必须认字（已修，实测 claimed）

**现象**：脚本跑完 `ok=false stage=verify`，日志停在「点了「立即领取」但未确认变成「今日已领」」，
`state_first=claim` / `state_after=claim` —— 说明**点了但没生效**，不是判状态错。

**一手证据**：`claim_blobs=[[46,168],[215,294]]`，点击坐标 `(107,1255)`；
而截图里该行的按钮「立即领取」实际在 **x≈215~294**（圆角按钮块），`(46,168)` 是左边的
**「今日可领100积分」文字**。旧代码取 `bl[0]` 中心 → **点到了文字上**，白点。

**根因**：`find_claim_btn()` 用「本行**首个**前景块中心」当按钮位置。这个假设只在
「立即领取 | 体验高校专家」两按钮同行时成立（那时首块确实是按钮）；9 期面板该行是
**文字 + 按钮**两颗，首块是文字。

**修法**（`buddy_checkin.py`，备份 `.bak-20260922-004358`）：
对该行**逐块 crop → 放大 2x → 单独 OCR 认字**，先取读出「立即领取」的那块；
没有则取读出「领取」的块；全读不到才退回首块（保留旧行为）。
结果落 `claim_blob_pick` / `claim_blob_text` 两个诊断字段。
修后实测 **70.1s 一次命中，status=claimed**，按钮变「今日已领」（截图肉眼验过）。

**泛训（可迁移）**
- **定位可点元素必须认字（内容匹配），不能靠块序/位置猜。** 块序随 UI 文案一变就错，
  而且错了**不报错**（点在文字上无声无息），只在最后"验证没变"时才暴露。
- 「点了但没生效」类失败，第一件事是**回看点击坐标落在哪个元素上**，
  别去怀疑 OCR 判状态 / 网络 / 面板刷新慢。
