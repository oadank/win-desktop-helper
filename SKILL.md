...
0. **🔴 Electron/大 DOM 应用（WorkBuddy/VSCode 系/网页壳/聊天类）禁用 /ui/tree、/ui/find(不带 name 的宽查)、/ui/readall** —— UIA 全树物化在 Electron 上永久阻塞且泄漏工作线程（实测 WorkBuddy 烧 1.1 核 70 分钟、整条工具链冻屏，服务重启才救回）。这些窗口只允许：ui_find 带 name=(服务端精确过滤不遍历全树)、/shot 截图+坐标操作。服务端有 UiCall 8s 硬超时+3 泄漏熔断兜底，触发熔断=所有 /ui/* 拒绝直到重启
