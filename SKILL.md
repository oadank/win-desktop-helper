...
## Win+Z 布局键位（微软官方口径 + 本机实测）

**官方规则（support.microsoft.com "Snap Your Windows"）**：
- 飞出条布局集合**动态**：随屏幕尺寸/窗口大小变，官方明说 "you might see different predetermined layouts"
- **数字键 = 飞出条里第几个格子**（行优先：从左到右、从上到下），不是固定语义
- 选完布局后，**zone 数字 = 该布局内的区域序号**（同样行优先）
- 选完布局 **Snap Assist 自动触发** → 点选其它窗填满剩余 → 自动成 **Snap group**（任务栏/Alt+Tab 可整组唤回）
- **拖分隔条联动**：拖一边，相邻贴靠窗一起缩放（吸附组核心特性）

**本机 24H2 2560 宽快照（3×3 九格，仅当前参考，非固定）**：
| # | 形状 | zone |
|---|---|---|
| 1 | 左右两半 | 1左 2右 |
| 2 | 左大右窄 | 1左大 2右窄 |
| 3 | 左大+右上/右下 | 1左大 2右上 3右下 |
| 4 | 左右两半 | 1左 2右 |
| 5 | 左大右窄(更宽) | 1左大 2右窄 |
| 6 | 三列均分 | 1左 2中 3右 |
| 7 | 上下两半 | 1上 2下 |
